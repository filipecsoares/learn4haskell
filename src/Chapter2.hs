...
subList start end xs
  | start > end = []  -- Base case: start is greater than end, return an empty list
  | start < 0 || end < 0 = []  -- Base case: negative positions, return an empty list
  | start >= length xs = []  -- Base case: start position exceeds list length, return an empty list
  | end < start = []  -- Base case: end position is less than start position, return an empty list
  | otherwise = take (end - start + 1) (drop start xs)  -- Take elements from start to end (inclusive)
